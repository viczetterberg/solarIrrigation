...
def lab_temp():
	...
	if humidity is not None and temperature is not None and moisture is not None:
		return render_template("lab_temp.html",temp=temperature,hum=humidity,moist=moisture)
	else:
		return render_template("no_sensor.html")

@app.route("/lab_env_db", methods=['GET'])  #Add date limits in the URL #Arguments: from=2015-03-04&to=2015-03-05
def lab_env_db():
	temperatures, moistures, humidities, timezone, from_date_str, to_date_str = get_records()

	# Create new record tables so that datetimes are adjusted back to the user browser's time zone.
	time_adjusted_temperatures = []
	time_adjusted_moistures    = []
	time_adjusted_humidities   = []
	for record in temperatures:
		local_timedate = arrow.get(record[0], "YYYY-MM-DD HH:mm").to(timezone)
		time_adjusted_temperatures.append([local_timedate.format('YYYY-MM-DD HH:mm'), round(record[2],2)])

	for record in moistures:
                local_timedate = arrow.get(record[0], "YYYY-MM-DD HH:mm").to(timezone)
                time_adjusted_moistures.append([local_timedate.format('YYYY-MM-DD HH:mm'), round(record[2],2)])
...
def get_records():
	...
	curs.execute("SELECT * FROM moistures WHERE rDateTime BETWEEN ? AND ?", (from_date_utc.format('YYYY-MM-DD HH:mm'), to_date_utc.format('YYYY-MM-DD HH:mm')))
        moistures                   = curs.fetchall()

	conn.close()

	return [temperatures, moistures, humidities, timezone, from_date_str, to_date_str]
